...
# Increment = 5
correlog_W_raw_incr5 <- correlog(data$latitude, data$longitude, data$wolffia_sp, latlon=TRUE, increment=5, resamp=500)
correlog_W_raw_incr5
plot(correlog_W_raw_incr5)
...
#######################
# Increment = 20
correlog_W_resid_incr20 <- correlog(data$latitude, data$longitude, best_glm_W_scaled_out_reduc5_resid, latlon=TRUE, increment=20, resamp=500)
correlog_W_resid_incr20
plot(correlog_W_resid_incr20)

# Increment = 10
correlog_W_resid_incr10 <- correlog(data$latitude, data$longitude, best_glm_W_scaled_out_reduc5_resid, latlon=TRUE, increment=10, resamp=500)
correlog_W_resid_incr10
plot(correlog_W_resid_incr10)

# Increment = 5
correlog_W_resid_incr5 <- correlog(data$latitude, data$longitude, best_glm_W_scaled_out_reduc5_resid, latlon=TRUE, increment=5, resamp=500)
correlog_W_resid_incr5
plot(correlog_W_resid_incr5)

# Increment = 2
correlog_W_resid_incr2 <- correlog(data$latitude, data$longitude, best_glm_W_scaled_out_reduc5_resid, latlon=TRUE, increment=2, resamp=500)
...
ggsave("Correlogram - Wolffia sp - incr10 - scaled_out_reduc5.jpg",correlogram_W_incr10,height=8,width=11)
...
################################
library(gridExtra)
all_sp_correlograms <- arrangeGrob(correlogram_W_incr10,correlogram_SP_incr10,correlogram_W_incr10,
             correlogram_W_incr20,correlogram_W_incr20,correlogram_W_incr20, 
             ncol=3)
