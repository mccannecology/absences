...
jpeg("Map - FPrichness - WA.jpg",height=800,width=1200,quality=100)

map(database= "state", "Washington", col="gray70", fill=TRUE, projection="azequalarea", resolution=0,xlim=c(-124.5,-117.1),ylim=c(45.5,49))
...
# FP richness = 0
locations_FP0 <- subset(dataWA_FPonly, dataWA_FPonly$FP_richness==0)
...
points(coord_FP0, pch=19, col="white", cex=1) 

# FP richness = 1
locations_FP1 <- subset(dataWA_FPonly, dataWA_FPonly$FP_richness==1)
...
# FP richness = 2
locations_FP2 <- subset(dataWA_FPonly, dataWA_FPonly$FP_richness==2)
...
# FP richness = 3
locations_FP3 <- subset(dataWA_FPonly, dataWA_FPonly$FP_richness==3)
...
# THERE ARE NO LAKES WITH FP RICHNESS = 4 - TURN THIS SECTION OF CODE ON OF THIS CHANGES 
#
# FP richness = 4
# locations_FP4 <- subset(dataWA_FPonly, dataWA_FPonly$FP_richness==4)
...
# points(coord_FP4, pch=19, col="darkgreen", cex=4) 

# FP richness = 5
locations_FP5 <- subset(dataWA_FPonly, dataWA_FPonly$FP_richness==5)
coord_FP5 <- mapproject(locations_FP5$longitude, locations_FP5$latitude, proj="")  # Projects the points using the same projection used for the map.
points(coord_FP5, pch=19, col="black", cex=2) 
