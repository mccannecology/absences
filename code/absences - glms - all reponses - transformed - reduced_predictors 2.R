...
write.csv(all_glm_LM_trans_reduc[1:numb_models],file="all_glm_LM_trans_reduc2.csv",na="NA")
save(list = ls(all = TRUE), file = "workspace - all glms dredge - transformed - reduced2.RData")
...
write.csv(all_glm_SP_trans_reduc[1:numb_models],file="all_glm_SP_trans_reduc2.csv",na="NA")
save(list = ls(all = TRUE), file = "workspace - all glms dredge - transformed - reduced2.RData")
...
write.csv(all_glm_W_trans_reduc[1:numb_models],file="all_glm_W_trans_reduc2.csv",na="NA")
save(list = ls(all = TRUE), file = "workspace - all glms dredge - transformed - reduced2.RData")
...
write.csv(all_glm_FPpres_trans_reduc[1:numb_models],file="all_glm_FPpres_trans_reduc2.csv",na="NA")
save(list = ls(all = TRUE), file = "workspace - all glms dredge - transformed - reduced2.RData")
...
write.csv(all_glm_FPrich_trans_reduc[1:numb_models],file="all_glm_FPrich_trans_reduc2.csv",na="NA")
save(list = ls(all = TRUE), file = "workspace - all glms dredge - transformed - reduced2.RData")
...
avg_W_trans_reduc <- model.avg(model_list_W)
avg_FPpres_trans_reduc <- model.avg(model_list_FPpress)
avg_FPrich_trans_reduc <- model.avg(model_list_FPrich)

# get the table for with the coefficient and p-value for each variable 
summary(avg_LM_trans_reduc)
...
importance(avg_LM_trans_reduc)
importance(avg_SP_trans_reduc)
importance(avg_W_trans_reduc)
importance(avg_FPpres_trans_reduc)
...
confint(avg_LM_trans_reduc)
confint(avg_SP_trans_reduc)
confint(avg_W_trans_reduc)
confint(avg_FPpres_trans_reduc)
