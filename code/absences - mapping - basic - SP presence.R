...
jpeg("Map - SPpresence.jpg",height=800,width=1200,quality=100)
...
locations <- cbind(dataSPACE,dataFP$spirodela_polyrhiza)
colnames(locations)[3] <- "spirodela_polyrhiza"

# Spirodela polyrhiza = 0
locations_FP0 <- subset(locations, locations$spirodela_polyrhiza==0)
...
locations_SP1 <- subset(locations, locations$spirodela_polyrhiza==1)
coord_FP1 <- mapproject(locations_SP1$longitude, locations_SP1$latitude, proj="")  # Projects the points using the same projection used for the map.
