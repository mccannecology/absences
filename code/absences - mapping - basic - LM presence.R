...
jpeg("Map - LMpresence.jpg",height=800,width=1200,quality=100)
...
locations <- cbind(dataSPACE,dataFP$lemna_minor)
colnames(locations)[3] <- "lemna_minor"

# Lemna minor = 0
locations_FP0 <- subset(locations, locations$lemna_minor==0)
...
# Lemna minor = 1
locations_LM1 <- subset(locations, locations$lemna_minor==1)
coord_FP1 <- mapproject(locations_LM1$longitude, locations_LM1$latitude, proj="")  # Projects the points using the same projection used for the map.
