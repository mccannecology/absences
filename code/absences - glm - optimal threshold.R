...
#ls()
#rm(list=objects(pattern="^formula"))
#rm(list=objects(pattern="^all"))
#rm(list=objects(pattern="^glm"))
#rm(list=objects(pattern="^best"))

#save.image("C:/Users/Mike/Desktop/Dropbox/absences/workspace - avg glm - opt_threshold.RData")

#########################
# Build new data frames #
#########################
data_LM_threshold <- data.frame(seq(1,nrow(temp_data_LM),1),temp_data_LM$LM,predict(avg_LM_trans, temp_data_LM, type='response'))
colnames(data_LM_threshold) <- c("site","obsv","pred")

data_SP_threshold <- data.frame(seq(1,nrow(temp_data_SP),1),temp_data_SP$SP,predict(avg_SP_trans, temp_data_SP, type='response'))
colnames(data_SP_threshold) <- c("site","obsv","pred")

data_W_threshold <- data.frame(seq(1,nrow(temp_data_W),1),temp_data_W$W,predict(avg_W_trans, temp_data_W, type='response'))
colnames(data_W_threshold) <- c("site","obsv","pred")

data_FPpres_threshold <- data.frame(seq(1,nrow(temp_data_FPpres),1),temp_data_FPpres$FPpres,predict(avg_FPpres_trans, temp_data_FPpres, type='response'))
